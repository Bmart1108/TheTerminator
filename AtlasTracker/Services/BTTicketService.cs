...
namespace AtlasTracker.Services
{
    public class BTTicketService : IBTTicketService
    {
        ...
        public async Task<List<Ticket>> GetTicketsByUserIdAsync(string userId, int companyId)
        {
            ...
            try
            {
                if (await _roleService.IsUserInRoleAsync(btUser!, nameof(BTRole.Admin)))
                {
                    tickets = (await _projectService.GetAllProjectsByCompanyAsync(companyId))
                                                    .SelectMany(p => p.Tickets).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(btUser!, nameof(BTRole.Developer)))
                {
                    tickets = (await _projectService.GetAllProjectsByCompanyAsync(companyId))
                                                    .SelectMany(p => p.Tickets)
                                                    .Where(t => t.DeveloperUserId == userId || t.OwnerUserId == userId).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(btUser!, nameof(BTRole.Submitter)))
                {
                    tickets = (await _projectService.GetAllProjectsByCompanyAsync(companyId))
                                                    .SelectMany(t => t.Tickets).Where(t => t.OwnerUserId == userId).ToList();
                }
                else if (await _roleService.IsUserInRoleAsync(btUser!, nameof(BTRole.ProjectManager)))
                {
